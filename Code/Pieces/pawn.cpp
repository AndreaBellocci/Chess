...
void Pawn_BuildPossibleMoves(Pieces pawn_id, int pawn_pos, const Chess& the_game, Pieces my_king_id, PossibleMoves& out_moves, bool allow_checks)
{
	...
	const int row_delta = (first_black_piece_id <= my_king_id && my_king_id <= last_black_piece_id) ? BOARD_SIDE : -BOARD_SIDE;
	bool moved = false;
	...
	if (the_game.m_EnPassantTargetPos != INVALID)
	{
		// m_EnPassantTargetPos stores the index of the square "behind" the pawn that just moved by 2
		const int en_passant_row = the_game.m_EnPassantTargetPos / BOARD_SIDE;
		const int en_passant_col = the_game.m_EnPassantTargetPos % BOARD_SIDE;
		...
		if ((en_passant_row == row - 1 || en_passant_row == row + 1) && (en_passant_col == col - 1 || en_passant_col == col + 1))
		{
			...
			if (!allow_checks)
			{
				...
				b2[the_game.m_EnPassantTargetPos] = b2[pawn_pos]; // Capturing pawn would move to board.m_EnPassantIndex
				...
				b2[the_game.m_EnPassantTargetPos + ((en_passant_row == 2) ? BOARD_SIDE : -BOARD_SIDE)] = nullptr;
			...
			if (king_ok)
			{
				DebugHelper(pawn_id, pawn_pos, the_game.m_EnPassantTargetPos, "capture en-passant", true, true);
				out_moves.push_back(the_game.m_EnPassantTargetPos);
			}
			else
			{
				DebugHelper(pawn_id, pawn_pos, the_game.m_EnPassantTargetPos, "capture en-passant", true, false);
			}
		}
	} // En passant
...
bool Pawn_CanEatKingInSquare(int pawn_pos, int enemy_king_pos, const board_t& board)
{
	...
	// Black pawns move down, white pawns move up, so row delta is positive for black and negative for white
	const int row_delta = (first_black_piece_id <= pawn_id && pawn_id <= last_black_piece_id) ? BOARD_SIDE : -BOARD_SIDE;
...
bool Pawn_CanMove(int pawn_pos, const Chess& the_game, Pieces my_king_id, int my_king_pos, bool allow_checks)
{
	...
	return (
		...
		(the_game.m_EnPassantTargetPos != INVALID && CanPieceMoveTo(the_game.m_EnPassantTargetPos, pawn_pos, the_game, my_king_id, my_king_pos, allow_checks)) // En passant
	);
} // Pawn_CanMove